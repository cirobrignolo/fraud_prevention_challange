...
- Me tome la libertad de crear las 2 funciones que traen la informacion pedida, tanto la cantidad de pagos rechazados en el último día, get_rejected_payments, y monto acumulado total (en usd) de pagos por usuario en la última semana, calculate_total_amount, para que en lugar de tener fijo los dias, se le envie una valor que puede ser modificado desde view.py. Esto en caso de que se quiera modificar se podria agregar como parametro de la llamada de fraud_check en lugar de dejarlo como un valor fijo en el archivo.
...
Donde from=ARS, to=USD y amount=1000 son los parametros puestos como ejemplo.
Los casos de respuesta son los siguientes.
.Caso positivo:
{"success":true,"query":{"from":"ARS","to":"USD","amount":1000},"info":{"timestamp":1729291997,"rate":0.00102},"date":"2024-10-18","result":1.02}
.Caso negativo:
{"success":false,"error":{"code":402,"type":"invalid_from_currency","info":"You have entered an invalid \"from\" property. [Example: from=EUR]"}} 
